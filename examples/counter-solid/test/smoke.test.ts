/**
 * End-to-end smoke test for the counter-solid example.
 *
 * Solid uses ReactiveSink rather than the VDOM-style DOMSink, so the
 ...
 * the count signal has propagated. Catches Solid-specific renderer wiring.
 */
// @vitest-environment jsdom
import { describe, expect, it } from "@effect/vitest"
import { Effect, Fiber, Layer } from "effect"
// `app` ends with `sink.render(...)` which returns synchronously in the
// Solid renderer (mounts once + returns the dispose handle). If we let the
// app fiber finish, Effect.provide closes the layer scope and the
// fromStream drainer fork is interrupted before the click can propagate.
// Wrapping with `Effect.andThen(Effect.never)` keeps the scope alive so
// the smoke test can observe the post-click DOM update.
import { DOMConfig } from "effect-cycle-dom"
import { ReactiveSinkLive } from "effect-cycle-solid"
import { TestDOMSource } from "effect-cycle-testing"
import app from "../src/App.js"

describe("counter-solid smoke", () => {
  it.effect("composes drivers and updates the DOM after a click", () =>
    Effect.gen(function* () {
      ...
      // the layer scope which fires Solid's dispose() finalizer and clears
      // the rendered DOM. So capture observed-during-polling and assert
      // after, then interrupt for cleanup.
